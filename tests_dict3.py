...
def setUpHeader(self):
    self.hand = PokerStarsHand(self.handfile, parse=False)
    self.hand.parse_header()

def setUpBody(self):
    self.hand = PokerStarsHand(self.handfile)

#http://stackoverflow.com/a/2799009/720077
def make_tests(dictionary):
    setattr(TestHeaderValues, 'setUp', setupHeader)
    setattr(TestHeaderValues, 'handfile', dictionary['file'])
    for attribute, result in dictionary['header_results'].items():
    ...
    setattr(TestBodyValues, 'setUp', setupBody)
    for attribute, result in dictionary['body_results'].items():
        test_method = create_test(attribute, result)
