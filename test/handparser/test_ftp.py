...
import pytz
from poker.handhistory import FullTiltHandHistory
...
def hand_header(request):
    ...
    h = FullTiltHandHistory(request.instance.hand_text, parse=False)
    h.parse_header()
    return h
...
def hand(request):
    ...
    return FullTiltHandHistory(request.instance.hand_text)
